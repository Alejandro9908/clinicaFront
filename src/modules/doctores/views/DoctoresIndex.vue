...
const { doctores, fetchDoctores, metadata, deleteDoctor } = useDoctores();
...
const searchRegistros = async () => {
  await fetchDoctores(search.value, currentPage.value, pageSize.value);
  rows.value = doctores.value.map(item => ({
    key: item.id,
    ID: item.id,
    Nombres: item.nombre,
...
const eliminar = async (id) => {
  ...
  try {
    await deleteDoctor(id) // espera que se elimine en el backend
...
<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center mb-4 border-b border-gray-200">
      <PageHeading title="Doctores" />

      <RouterLink
          :to="{ path: '/doctores/create' }"
